...
const leerLog= async (filePath, periodo, mina)=>{
    ...
    //Con esta linea diferenciaremos en los logs para analizar segun sea de CerroVerde o de LasBambas
    const mensaje= (mina==='CVE')?' ClassicDispatchAdapter':'';
    ...
    return new Promise(resolve=>{
        ...
        rd.on('line', function(line) {
            ...
            if(line.toString().includes('Commiting'+mensaje) || line.toString().includes('Uncommiting'+mensaje)){
                //console.log(line);
                //console.log(line.slice(1,20))
                if(periodo && date.getTime()>= periodo[0].getTime() && date.getTime()<= periodo[1].getTime()){
                    const state= line.toString().includes('Commiting'+mensaje) ? 'start' : 'stop';
